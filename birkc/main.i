...
#line 1 "c:\\users\\aleksander\\documents\\projects\\birkc\\birk.h"
...
#line 6 "c:\\users\\aleksander\\documents\\projects\\birkc\\birk.h"
...
#line 924 "C:\\Program Files (x86)\\Microsoft Visual Studio 12.0\\VC\\INCLUDE\\stdlib.h"
...
#line 7 "c:\\users\\aleksander\\documents\\projects\\birkc\\birk.h"
#line 1 "C:\\Program Files (x86)\\Microsoft Visual Studio 12.0\\VC\\INCLUDE\\stdbool.h"
...
#line 15 "C:\\Program Files (x86)\\Microsoft Visual Studio 12.0\\VC\\INCLUDE\\stdbool.h"

#line 17 "C:\\Program Files (x86)\\Microsoft Visual Studio 12.0\\VC\\INCLUDE\\stdbool.h"





#line 8 "c:\\users\\aleksander\\documents\\projects\\birkc\\birk.h"
...
#line 9 "c:\\users\\aleksander\\documents\\projects\\birkc\\birk.h"
...
typedef struct FileData FileData; struct FileData
{
	isize size;
	u8* data;
};

FileData birk_read_entire_file(_Bool newline, char *path);
void birk_free_file(FileData fd);



_Bool birk_is_uppercase(int c);
_Bool birk_is_lowercase(int c);
_Bool birk_is_alpha(int c);
_Bool birk_is_alnum(int c);
_Bool birk_is_digit(int c);

_Bool birk_strncmp(char *a, char *b, isize n);
...
typedef enum TokenDefType TokenDefType; enum TokenDefType
{
	TokenDefKeyword = 0,
	TokenDefToken,
};

typedef enum BirkTokenType BirkTokenType; enum BirkTokenType
{
	TokenUnknown = 0,
	TokenNumber,
	TokenIdent,
...
typedef struct TokenDef TokenDef; struct TokenDef {
	int type;
	int id;
...
typedef struct Token Token; struct Token
{
	int type;
	char *text;
	isize len;
...
typedef struct Lexer Lexer; struct Lexer
{
	TokenDef *token_defs;
	isize def_count;
	char *data;
	isize data_size;
	_Bool eat_whitespace;
	_Bool parse_string;

	char *curr, *end;
};

typedef enum TokenDefsResult TokenDefsResult; enum TokenDefsResult
{
	TokenDefsValid = 0,
...
void birk_lexer_init(Lexer *l, char *data, isize data_size, TokenDef *token_defs, isize def_count, _Bool eat_whitespace, _Bool parse_string);
void birk_lexer_eat_whitespace(Lexer *l);
Token birk_lexer_get_token(Lexer *l);

#line 139 "c:\\users\\aleksander\\documents\\projects\\birkc\\birk.h"
...
FileData birk_read_entire_file(_Bool zero, char *path)
{
	FileData fd = {0};
...
_Bool birk_is_uppercase(int c) { return ((c >= 'A') && (c <= 'Z')); }
_Bool birk_is_lowercase(int c) { return ((c >= 'a') && (c <= 'z')); }
_Bool birk_is_alpha(int c) { return birk_is_lowercase(c) || birk_is_uppercase(c); }
_Bool birk_is_alnum(int c) { return birk_is_alpha(c) || birk_is_digit(c); }
_Bool birk_is_digit(int c) { return ((c >= '0' && (c <= '9'))); }
...
Token birk_lexer_get_token(Lexer *l)
{
	...
	if(!l->eat_whitespace) {
		switch(c) {
			...
			case ' ': {
				l->curr++;
				return (Token){TokenSpace, 0, 0};
			} break;
			case '\t': {
				l->curr++;
				return (Token){TokenTab, 0, 0};
			} break;
			case '\n': {
				l->curr++;
				return (Token){TokenNewline, 0, 0};
			} break;
		}
	}
	
	for(int i = 0; i < l->def_count; i++) {
	...
	l->curr++;
	return (Token){TokenUnknown, 0, 0};
}

#line 441 "c:\\users\\aleksander\\documents\\projects\\birkc\\birk.h"
#line 3 "main.c"

typedef enum TokenType TokenType; enum TokenType
{
...
TokenDef token_defs[] = {
	...
	{TokenDefKeyword, TokenIs, "is", 0, 0, 0, 0},
	{TokenDefToken, TokenOpenBrace, 0, '{', 0, 0, 0},
	{TokenDefToken, TokenCloseBrace, 0, '}', 0, 0, 0},
	{TokenDefToken, TokenSemicolon, 0, ';', 0, 0, 0},
};

typedef struct ChildStruct ChildStruct; struct ChildStruct
{
	char *parent_name;
	Token *members;
};

typedef struct BaseStruct BaseStruct; struct BaseStruct
{
	char *name;
	Token **members;
...
int main(int argc, char **argv)
{
	...
	int index = 0;
	TokenDefsResult result = birk_lexer_validate_token_defs(token_defs, (sizeof(token_defs) / sizeof(token_defs[0])), &index);
	if(result != TokenDefsValid) {
		printf("TokenDefs not valid: %d at %d\n", result, index);
		return 1;
	}	

	FileData fd = birk_read_entire_file(0, argv[1]);
	Lexer l = {0};
	birk_lexer_init(&l, fd.data, fd.size, token_defs, (sizeof(token_defs) / sizeof(token_defs[0])), 1, 1);
	...
	while(t.type != TokenEof) {
		if(t.type == TokenBase) {
			...
			if(name.type != TokenIdent) {
				...
				do {birk_free_file(fd); return 1;} while(0);
			}

			do {if(birk_lexer_get_token(&l).type != TokenOpenBrace) { printf("Expected %d!\n", TokenOpenBrace); do {birk_free_file(fd); return 1;} while(0); }} while(0);
			...
			while(t.type != TokenCloseBrace) {
				...
				while(t.type != TokenSemicolon) {
					((((member)==0 || ((int *) (member) - 2)[1]+((1)) >= ((int *) (member) - 2)[0]) ? ((member) = birk_array_grow_func((member), (1), sizeof(*(member)))) : 0), (member)[((int *) (member) - 2)[1]++] = (t));
					t = birk_lexer_get_token(&l);
				}
				((((base.members)==0 || ((int *) (base.members) - 2)[1]+((1)) >= ((int *) (base.members) - 2)[0]) ? ((base.members) = birk_array_grow_func((base.members), (1), sizeof(*(base.members)))) : 0), (base.members)[((int *) (base.members) - 2)[1]++] = (member));
				t = birk_lexer_get_token(&l);
			}
			do {if(birk_lexer_get_token(&l).type != TokenSemicolon) { printf("Expected %d!\n", TokenSemicolon); do {birk_free_file(fd); return 1;} while(0); }} while(0);
	...
	BaseStruct it;
	for(int it_index = 0, it = base_structs[0]; it = base_structs[it_index], it_index < ((base_structs) ? ((int *) (base_structs) - 2)[1] : 0); it_index++) {
		printf("typedef struct %s %s;\n", it.name, it.name);
		printf("struct %s {"\n), it.name;

		Token *it;
		for(int it_index = 0, it = it.members[0]; it = it.members[it_index], it_index < ((it.members) ? ((int *) (it.members) - 2)[1] : 0); it_index++) {
			Token token;
			for(int it_index = 0, it = it[0]; it = it[it_index], it_index < ((it) ? ((int *) (it) - 2)[1] : 0); it_index++) {
				printf("%s", it.text);
			}
			printf(";\n");
		}
	...
	do {birk_free_file(fd); return 0;} while(0);
}
